import { Scene as m, WebGLRenderer as g, sRGBEncoding as y, PerspectiveCamera as w, Mesh as E, MeshStandardMaterial as M, Group as p, BufferGeometry as A, BufferAttribute as b } from "three";
import { CSS3DRenderer as x } from "three/addons/renderers/CSS3DRenderer.js";
import { C as I } from "./controller-aK7fMUmA.js";
import { U as R } from "./ui-F0VAZ3n3.js";
const S = { BufferGeometry: A, BufferAttribute: b };
class P {
  constructor({
    container: a,
    uiLoading: t = "yes",
    ...
    this.container = a, this.ui = new R({ uiLoading: t, uiScanning: o, uiError: n }), this.controller = new I({
      filterMinCF: e,
      filterBeta: h
    }), this.disableFaceMirror = l, this.scene = new m(), this.cssScene = new m(), this.renderer = new g({ antialias: !0, alpha: !0 }), this.cssRenderer = new x({ antialias: !0 }), this.renderer.outputEncoding = y, this.renderer.setPixelRatio(window.devicePixelRatio), this.camera = new w(), this.userDeviceId = s, this.environmentDeviceId = c, this.anchors = [], this.faceMeshes = [], this.latestEstimate = null, this.container.appendChild(this.renderer.domElement), this.container.appendChild(this.cssRenderer.domElement), this.shouldFaceUser = !0, window.addEventListener("resize", this._resize.bind(this));
  }
  async start() {
  ...
  addFaceMesh() {
    const a = this.controller.createThreeFaceGeometry(S), t = new E(a, new M({ color: 16777215 }));
...
window.MEDIAPIPEAR || (window.MEDIAPIPEAR = {});
window.MEDIAPIPEAR.FACE || (window.MEDIAPIPEAR.FACE = {});
window.MEDIAPIPEAR.FACE.MediapipeARThree = P;
export {
  P as MediapipeARThree
};
